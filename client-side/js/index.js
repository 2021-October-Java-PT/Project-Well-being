  ...
  app.addEventListener("click", (event) => {
    if (event.target.classList.contains("journal-submit-button")) {
      ...
      const journalEntryContent =
        event.target.parentElement.querySelector("#journal-entry").value;
      apiHelpers.postRequest(
...
function deleteReminder() {
  app.addEventListener("click", (event) => {
    if (event.target.classList.contains("reminder-delete")) {
      const deleteReminderId =
        event.target.parentElement.querySelector(".reminder-id").value;
      apiHelpers.deleteRequest(
        `http://localhost:8080/api/reminders/${deleteReminderId}/delete-reminder`,
        () => {
...
function deleteJournal() {
  app.addEventListener("click", (event) => {
    if (event.target.classList.contains("journal-delete")) {
      const deleteJournalId =
        event.target.parentElement.querySelector(".journal-id").value;
      apiHelpers.deleteRequest(
        `http://localhost:8080/api/journals/${deleteJournalId}/delete-journal-entry`,
        (journals) => {
          app.innerHTML = Journal(journals);
        }
      );
    ...
    if (event.target.classList.contains("articleImg1")) {
      ...
      apiHelpers.getRequest(
        ...
        (reminders) => {
          ...
          list.insertAdjacentHTML("beforeend", Charts());
...
// function goToCharts() {
//   app.addEventListener("click", (event) => {
//     if (event.target.classList.contains("charts")) {
//       app.innerHTML = Charts();
//     }
//   });
// }
...
function search() {
  ...
  searchSubmit.addEventListener("click", () => {
    ...
    console.log(value);
    apiHelpers.getRequest(
      `https://health.gov/myhealthfinder/api/v3/topicsearch.json?keyword=${value}`,
      (resources) => {
